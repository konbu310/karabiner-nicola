...
function createRightShiftBasedShortcut(rightShiftKey: string) {
...
export async function generate(config: Config): Promise<string> {
  const base = {
    title: "Enable NICOLA with Karabiner",
    rules: [
      {
        description: "Karabiner NICOLA",
        manipulators: [
          ...createRightShiftBasedShortcut(config.rightShift),
        ] as any[],
      },
    ],
  };
